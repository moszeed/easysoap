    ...
    var request = {};
        ...
        request.request = function(attrs, params, opts) {
            ...
            var template_params = {};
                ...
                template_params.methodNs    = attrs.methodNs    || false;
            ...
            function _request(tpl) {
                ...
                var headers = { 'SOAPAction' : attrs.SOAPAction, 'Content-type' : 'text/xml' };
                if( params.headers ) for( var i in params.headers ) headers[i] = params.headers[i];
                if( attrs.headers ) for( var i in attrs.headers ) headers[i] = attrs.headers[i];
                console.log( headers );
                ...
                var request  = protocol.request({
                    ...
                    request.once('response',  function(res) {
                        if( res.statusCode != 200 ) {
                            def.reject( new Error( 'Invalid HTTP Response' ));
                            return ;
                        }
