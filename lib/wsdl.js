(function() {
    ...
    function nodeDataExtract(node, param) {

        var data = {};

        if (node.children           === void 0 ||
            node.children.length    === 0) {
            if( param ) if( param.type == 'base64Binary' ) {
                node.val = new Buffer( node.val, 'base64' );
            }
    ...
    Handler.prototype.getMethodParams = function(params, wsdl, opts) {
        ...
        if (this._cache[opts.path][params.method]   === void 0) {
            ...
            var bindings    = searchNode(xml,   'binding');
            var operationsB = bindings.length > 0 ? searchNode(bindings[0],  'operation') : null ;
            var soapAct     = undefined;

            if( operationsB ) _(operationsB).each(function(operation) {

                if (params.method == getName(operation.attr.name)) {
                    var operationOp = searchNode( operation, 'operation' );
                    if( operationOp.length > 0 ) soapAct = operationOp[0].attr.soapAction;
            ...
            this._cache[opts.path][params.method] = {
                ...
                'response'  :   response,
                'action'    :   soapAct,
                'schema'    :   schema[0].attr.targetNamespace
            };
    ...
    Handler.prototype.responseToArray = function(params) {
        ...
        _(this._cache[params.path][params.method].response.params).each(function(item) {
            ...
            _(node_parts).each(function(part) {
                node_data.push(nodeDataExtract(part, item));
            });
        });
